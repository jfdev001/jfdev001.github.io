...
\
[jfdev001.github.io/talks/git-for-everyone/index.html](https://jfdev001.github.io/talks/git-for-everyone/index.html#/title-slide)
...
from [install on Windows](https://git-scm.com/install/windows). The default settings are fine.
...
- Git is not just a series of backups, it facilitates collaborative software
development and clean version tracking.
...
# Git Rabbit Hole {.smaller}

:::: {.columns}
::: {.column width=70%}
- `git merge` (preserve commit history)
- `git rebase` (graft commit on tip of other branch)
- `git worktree` (two branches "active" at the same time)
- `git filter-branch` (for altering history, e.g., remove large files)
- `git cherry-pick` (take part of other commits)
- `git commit --amend` (change old commit messages)
- `git reset --soft HEAD~<ncommits>` (squash commits)
- [Code Refinery Lessons](https://coderefinery.org/lessons/), [Pro Git Book](https://git-scm.com/book/en/v2), open source contribution guidelines
- ...
:::
...
# NOTE: The first time you do this, SSH will prompt you to save the fingerprint
# of the host, just hit yes and ignore the warnings.
# The result of this command should be "You've successfully authenticated"
ssh -T git@github.com
```

## Git/GitHub Contribution Sample Workflow {.smaller}

```shell
# Fork the repository on GitHub first (e.g.,
# git@github.com:mygithubuser/miniWeather.git

# clone your fork locally
git clone git@github.com:mygithubuser/miniWeather.git
cd miniWeather

# create a new branch for your contribution
# recommended naming scheme (issue number may not be applicable):
# <myname>/<issue-number>-<feature-description>
git checkout -b myname/42-add-weather-parser

# make your code changes, then stage them
git add .

# commit with a clear message
git commit -m "Add weather parser for METAR data"

# push the branch to your fork
git push -u origin myname/42-add-weather-parser

# go to your fork on GitHub and open a Pull Request (PR)
# GitHub will usually show a "Compare & pull request" button
# if your fork goes out of sync with the upstream, just click on "Sync fork" on
# GitHub, and then do `git pull` locally
```
