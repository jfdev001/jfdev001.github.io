...
- `git merge` (same thing as GitHub graphical merge, preserve commit history)
- `git rebase` (similar to merge, graft your commits onto tip of other branch)
- `git worktree` (have two branches "active" at the same time)
- `git filter-branch` (for altering history, e.g., remove large files)
- `git cherry-pick` (take part of other commits)
- `git commit --amend` (change commit old commit messages)
- `git reset --soft HEAD~<ncommits>` (can be used to squash commits)
